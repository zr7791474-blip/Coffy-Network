/* ============ IMAGES (référence — remplacer par un reportage photo maison) ============ */
var IMG = {
...
function renderMenu(navId, catsId){
  ...
  MENU.forEach(function(cat, idx){
    ...
    b.textContent = (lang === 'ar' && cat.nameAr) ? cat.nameAr : cat.name;
    ...
    if(cat.img && IMG[cat.img]){
      html += '<div class="menu-feat"><div class="thumb"><img src="'+IMG[cat.img]+'" alt="'+cat.name+'" loading="lazy"></div>';
    ...
    if(cat.foot){ html += '<p class="footnote">'+(lang==='ar' ? '*نسخة ساخنة — مرفقة بقارورة مياه معدنية صغيرة سيدي علي 33 سل.' : cat.foot)+'</p>'; }
    sec.innerHTML = html;
...
var I18N = {
  fr: {
    ...
    loc_hours_label:"Horaires", loc_hours_days:"Tous les jours", loc_contact_label:"Contact", loc_itin:"Itinéraire →",
    ...
    foot_privacy:"Confidentialité", foot_terms:"Conditions générales", foot_legal_note:"Mentions légales disponibles sur demande.",
  ...
  ar: {
    ...
    loc_hours_label:"أوقات العمل", loc_hours_days:"كل يوم", loc_contact_label:"للتواصل", loc_itin:"المسار ←",
    ...
    foot_privacy:"الخصوصية", foot_terms:"الشروط العامة", foot_legal_note:"المعلومات القانونية الكاملة متوفرة عند الطلب.",
